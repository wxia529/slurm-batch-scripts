...
7. VASP 版本固定为 `6.5.1-vtst`
8. 支持 `std`、`gam`、`ncl`，默认使用 `std`
9. Slurm 作业名称使用当前任务目录名称
10. 标准输出追加写入任务目录中的 `log`
11. 标准错误写入任务目录中的 `vasp.err`
12. 启动作业前覆盖重建 `PATH` 和 `LD_LIBRARY_PATH`，不在用户原环境后追加
13. `PATH` 固定为：
...
14. `LD_LIBRARY_PATH` 固定为：

    ```text
    /opt/slurm/slurm/lib:/lib64:/usr/lib64:/usr/local/lib64
    ```

15. 路径重建后加载 `/public3/home/sc71468/soft/toolchain/vasp651.env`
16. 固定设置 `OMP_NUM_THREADS=1`
17. 使用 `mpirun -n <节点数乘以 64> vasp_<运行类型>` 启动作业
18. Slurm 成功接收作业后追加任务目录中的 `Batch.log`
