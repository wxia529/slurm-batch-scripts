...
usage() {
    echo "Usage: myneb.sh <input_file> [nodes]"
}

if (( $# < 1 || $# > 2 )); then
    echo "Error: A Quantum ESPRESSO neb.x input file and optional node count are required." >&2
    usage
    exit 1
fi
if [[ ! -f "$1" ]]; then
    echo "Error: NEB input file does not exist: $1" >&2
    exit 1
fi
...
TMP_SCRIPT=$(mktemp "${TMPDIR:-/tmp}/myneb-${USER:-user}-${JOB_NAME}.XXXXXX")
trap 'rm -f "$TMP_SCRIPT"' EXIT

cat > "$TMP_SCRIPT" <<EOF
#!/usr/bin/env bash

echo "Starting QE neb.x job \$SLURM_JOB_ID at \$(date)"
...
# neb.x requires a named input option; stdin redirection is unsupported.
mpirun -n ${MPI_PROCESSES} neb.x -inp ${INPUT_NAME_Q} > ${OUTPUT_FILE_Q} 2> ${ERROR_FILE_Q}
STATUS=\$?
echo "QE neb.x job \$SLURM_JOB_ID finished with status \$STATUS at \$(date)"
exit \$STATUS
EOF
...
if ! SBATCH_OUTPUT=$(sbatch --parsable --job-name="$JOB_NAME" --nodes="$NODES" --ntasks="$MPI_PROCESSES" --partition="$PARTITION" "$TMP_SCRIPT"); then
    echo "Error: QE neb.x job submission failed." >&2
    exit 1
fi
