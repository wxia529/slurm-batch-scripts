...
usage() {
    echo "Usage: mypp.sh <input_file> [nodes]"
}

if (( $# < 1 || $# > 2 )); then
    echo "Error: A Quantum ESPRESSO pp.x input file and optional node count are required." >&2
    usage
    exit 1
fi
if [[ ! -f "$1" ]]; then
...
TMP_SCRIPT=$(mktemp "${TMPDIR:-/tmp}/mypp-${USER:-user}-${JOB_NAME}.XXXXXX")
trap 'rm -f "$TMP_SCRIPT"' EXIT

cat > "$TMP_SCRIPT" <<EOF
#!/usr/bin/env bash

echo "Starting QE pp.x job \$SLURM_JOB_ID at \$(date)"
...
mpirun -n ${MPI_PROCESSES} pp.x -inp ${INPUT_NAME_Q} > ${OUTPUT_FILE_Q} 2> ${ERROR_FILE_Q}
STATUS=\$?
echo "QE pp.x job \$SLURM_JOB_ID finished with status \$STATUS at \$(date)"
exit \$STATUS
EOF
...
if ! SBATCH_OUTPUT=$(sbatch --parsable --job-name="$JOB_NAME" --nodes="$NODES" --ntasks="$MPI_PROCESSES" --partition="$PARTITION" "$TMP_SCRIPT"); then
    echo "Error: QE pp.x job submission failed." >&2
    exit 1
fi
