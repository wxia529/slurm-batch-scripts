...
usage() {
    echo "Usage: myfd.sh <input_file> [nodes]"
}

if (( $# < 1 || $# > 2 )); then
    echo "Error: An FDVIB input file and optional node count are required." >&2
    usage
    exit 1
fi
if [[ ! -f "$1" ]]; then
...
TMP_SCRIPT=$(mktemp "${TMPDIR:-/tmp}/myfd-${USER:-user}-${JOB_NAME}.XXXXXX")
trap 'rm -f "$TMP_SCRIPT"' EXIT

cat > "$TMP_SCRIPT" <<EOF
#!/usr/bin/env bash

echo "Starting FDVIB job \$SLURM_JOB_ID at \$(date)"
...
mpirun -n ${MPI_PROCESSES} fdvib -inp ${INPUT_NAME_Q} > ${OUTPUT_FILE_Q} 2> ${ERROR_FILE_Q}
STATUS=\$?
echo "FDVIB job \$SLURM_JOB_ID finished with status \$STATUS at \$(date)"
exit \$STATUS
EOF
...
echo "Program       : fdvib"
echo "FDVIB output  : $INPUT_DIR/$OUTPUT_FILE"
...
if ! SBATCH_OUTPUT=$(sbatch --parsable --job-name="$JOB_NAME" --nodes="$NODES" --ntasks="$MPI_PROCESSES" --partition="$PARTITION" "$TMP_SCRIPT"); then
    echo "Error: FDVIB job submission failed." >&2
    exit 1
fi
...
RECORD="${TIMESTAMP} | FDVIB | job_id=${JOB_ID} | job=${JOB_NAME} | partition=${PARTITION} | nodes=${NODES} | processes=${MPI_PROCESSES} | directory=${INPUT_DIR} | input=${INPUT_NAME} | output=${OUTPUT_FILE}"
