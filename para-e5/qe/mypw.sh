...
usage() {
    echo "Usage: mypw.sh <input_file> [nodes]"
}

if (( $# < 1 || $# > 2 )); then
    echo "Error: A Quantum ESPRESSO input file and optional node count are required." >&2
    usage
    exit 1
fi
if [[ ! -f "$1" ]]; then
...
TMP_SCRIPT=$(mktemp "${TMPDIR:-/tmp}/mypw-${USER:-user}-${JOB_NAME}.XXXXXX")
trap 'rm -f "$TMP_SCRIPT"' EXIT

cat > "$TMP_SCRIPT" <<EOF
#!/usr/bin/env bash

echo "Starting QE job \$SLURM_JOB_ID at \$(date)"
...
mpirun -n ${MPI_PROCESSES} pw.x -inp ${INPUT_NAME_Q} > ${OUTPUT_FILE_Q} 2> ${ERROR_FILE_Q}
STATUS=\$?
echo "QE job \$SLURM_JOB_ID finished with status \$STATUS at \$(date)"
exit \$STATUS
EOF
